...
const blockIconURI = 'data:image/svg+xml;base64,';
...
class Scratch3PenBlocks {
    constructor (runtime) {
        ...
        /**
         * Cached 2D context for the bitmap canvas — avoids getContext() on every draw call.
         * Invalidated whenever the canvas is resized.
        ...
        /**
         * Cached reference to the bitmap skin object — avoids _allSkins[] lookup on every draw call.
         * @type {object|null}
         * @private
         */
        this._printSkin = null;
     ...
     * Called on every frame for every moving sprite with pen down — keep it lean.
    ...
    _onTargetMoved (target, oldX, oldY, isForce) {
        ...
        if (!isForce) {
            // Inline check avoids a function call on every sprite movement tick
            const penSkinId = this._penSkinId >= 0 ? this._penSkinId : this._getPenLayerID();
            if (penSkinId >= 0) {
    ...
    removeCameraBinding () {
        this.cameraBound = -1;
        // Only operate if the pen layer already exists — avoids creating it unnecessarily
        if (this._penSkinId >= 0) {
            this.runtime.renderer.penTranslate(this._penSkinId, 0, 0, 1, 0);
        }
    }
    ...
    printText (args) {
        ...
        // Build font string in one shot rather than through repeated concatenation
        ctx.font = `${this.printTextAttribute.italic ? 'italic ' : ''}${this.printTextAttribute.weight} ${this.printTextAttribute.size}px ${this.printTextAttribute.font}`;
    ...
    async _drawUriImage ({URI, X, Y, WIDTH, HEIGHT, ROTATE, CROPX, CROPY, CROPW, CROPH}) {
        ...
        // ensure all crop values exist; fix: original had CROPH && CROPH (typo for CROPW && CROPH)
        if (!(typeof CROPX === "number" && typeof CROPY === "number" && CROPW && CROPH)) {
            drawArgs.splice(0, 4);
        }
    ...
    /**
     * Upload the bitmap canvas contents to the pen skin and trigger a redraw.
     * Uses cached skin reference and pen skin ID to avoid redundant lookups.
     * @param {CanvasRenderingContext2D} ctx
     * @private
     */
    _drawContextToPen (ctx) {
        const { width, height } = this.bitmapCanvas;
        // No restore() needed — _getBitmapCanvas resets transform via setTransform() each call

        // Use cached _printSkin to skip the _allSkins[] array lookup on every draw
        this._printSkin._setTexture(ctx.getImageData(0, 0, width, height));
        this.runtime.renderer.penStamp(this._penSkinId, this.bitmapDrawableID);
        this.runtime.requestRedraw();
    }

    /**
     * Prepare the bitmap canvas for a drawing operation.
     *
     * KEY OPTIMISATION: the original code resized the canvas on every call to reset
     * its transform state. Canvas resize triggers GPU memory reallocation and
     * invalidates the 2D context — extremely wasteful for frequent draws.
     *
     * Instead we:
     *  - Only resize when the pen skin's actual dimensions change (rare).
     *  - Cache the 2D context and the print skin reference across calls.
     *  - Use setTransform() to reset the transform matrix cheaply, then re-apply
     *    the standard translate + scale. This is equivalent to a resize-reset at
     ...
     */
    _getBitmapCanvas () {
        // Ensure the pen layer (and bitmap canvas) exists
        if (this._penSkinId < 0) this._getPenLayerID();

        const penSkin = this.runtime.renderer._allSkins[this._penSkinId];
        const width = penSkin._size[0];
        const height = penSkin._size[1];

        // Only resize when the stage dimensions have actually changed.
        // Resizing a canvas causes a full GPU reallocation — avoid it every draw call.
        if (!this._bitmapCtx || this.bitmapCanvas.width !== width || this.bitmapCanvas.height !== height) {
            this.bitmapCanvas.width = width;
            this.bitmapCanvas.height = height;
            this._bitmapCtx = this.bitmapCanvas.getContext('2d');
            this._printSkin = this.runtime.renderer._allSkins[this.bitmapSkinID];
        }

        const ctx = this._bitmapCtx;
        // Reset transform to identity (replaces the resize trick) then clear and re-apply base transform
    ...
    _stamp (target) { // used by compiler
        // Inline the ID check — avoids a function call on every stamp operation
        const penSkinId = this._penSkinId >= 0 ? this._penSkinId : this._getPenLayerID();
        if (penSkinId >= 0) {
    ...
    _updatePenColor (penState) {
        ...
        const color4f = penState.penAttributes.color4f;
        color4f[0] = rgb.r / 255.0;
        color4f[1] = rgb.g / 255.0;
        color4f[2] = rgb.b / 255.0;
        color4f[3] = this._transparencyToAlpha(penState.transparency);
    }
    ...
    _getPenColor (target) {
        ...
        return Color.rgbToHex({
            ...
            g: color4f[1] * 255,
            b: color4f[2] * 255,
            a: this._alphaToTransparency(color4f[3])
        });
    }
    ...
    drawComplexShape (args, util) {
        ...
        // Direct index access is marginally faster than points.at(-1)
        const firstPos = points[points.length - 1];
    ...
    drawArrayComplexShape (args, util) {
        ...
        // Build point pairs via index — avoids the isXCoord boolean flag and per-iteration branch
        const count = providedPoints.length >> 1;
        const points = new Array(count);
        for (let i = 0; i < count; i++) {
            points[i] = {
                x: Cast.toNumber(providedPoints[i * 2]),
                y: Cast.toNumber(providedPoints[i * 2 + 1])
            };
        }
